...
class User extends ActiveRecord implements IdentityInterface
{
    ...
    {
        $redis = Yii::$app->redis;
        $key = "user:{$this->getId()}:subscribtions";
    ...
    {
        $redis = Yii::$app->redis;
        return $redis->scard("user:{$this->getId()}:subscribtions");
    }
    ...
    public function getMutualSubscriptionsTo(User $user)
    {
        ...
        $key1 = "user:{$this->getId()}:subscribtions";
    ...
    public function isFollowing(User $user)
    {
        $redis = Yii::$app->redis;
        $key = "user:{$this->getId()}:subscribtions";
        return $redis->sismember($key, $user->getId());
    }
